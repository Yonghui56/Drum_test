...
/**
  ...
  */
LocalProblem_EOS_NonIso_TotalMassEXP::LocalProblem_EOS_NonIso_TotalMassEXP()
{
...
/**
  ...
  */
LocalProblem_EOS_NonIso_TotalMassEXP::~LocalProblem_EOS_NonIso_TotalMassEXP(void)
{
 ...
 */
void LocalProblem_EOS_NonIso_TotalMassEXP::solve(ogsChem::LocalVector & Input, ogsChem::LocalVector & Output)
{
	if (Input.size() == 3 && Output.size() == 2)
	{		
		std::size_t m_flag = 1; 
		double rho_l_std = 1000.0;
		...
		if (m_flag == 0)
		{
			Output.head(2) = U_cur;
			if (U_cur(1) < 0){
				std::cout << "liquid saturation is negative" << std::endl;
		...
		else
		{
			...
			std::cout << Input << std::endl;
			Output(1) = _EOS->get_P_sat(Input(0), Input(2));
			Output(0) = (Input(1) - rho_l_std) / (Output(1) - rho_l_std);
		}
	}
	else
	{
...
/**
  ...
  */
void LocalProblem_EOS_NonIso_TotalMassEXP::solve_minimization(ogsChem::LocalMatrix & J,
...
void LocalProblem_EOS_NonIso_TotalMassEXP::solve_LocalProblem_DirectSolve(std::size_t & flag)
{
...
/**
  ...
  */
void LocalProblem_EOS_NonIso_TotalMassEXP::solve_LocalProblem_Newton_LineSearch(std::size_t & flag)
{
