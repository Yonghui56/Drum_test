...
>
class SingleStepCMP_NonIsoGlobalNCPForm
...
public:
	...
	*/
	SingleStepCMP_NonIsoGlobalNCPForm(MyDiscreteSystem*      dis,
	...
	*/
	virtual ~SingleStepCMP_NonIsoGlobalNCPForm()
	{
		/*
...
private:
	DISALLOW_COPY_AND_ASSIGN(SingleStepCMP_NonIsoGlobalNCPForm);
...
template <
	...
	class T_USER_NON_LINEAR_SOLUTION
>
SingleStepCMP_NonIsoGlobalNCPForm<T_USER_FUNCTION_DATA, T_USER_FEM_PROBLEM, T_USER_NON_LINEAR_PROBLEM, T_USER_NON_LINEAR_SOLUTION>
::SingleStepCMP_NonIsoGlobalNCPForm(MyDiscreteSystem*                dis,
                           UserFemProblem*                  problem,
						   UserNonLinearSolution*           nlin_solution,
						   UserFunctionData*                function_data)
...
_problem(problem), _nlin_solution(nlin_solution), _discrete_system(dis), _function_data(function_data)
{
	...
	INFO("->Setting up a solution algorithm for CMP_NonIsoGlobalNCPForm problem.");
		...
		for (i = 0; i < var->getNumberOfDirichletBC(); i++) {
			...
			for (j = 0; j < list_bc_nodes.size(); j++)
			{
				...
				if (i_var == 0)
					...
					_function_data->set_X_G_a_node_values(node_id, node_value);
				else if (i_var == 2)
					_function_data->set_X_G_w_node_values(node_id, node_value);
				else if (i_var==3)
					_function_data->set_S_node_values(node_id, node_value);
				else if (i_var == 4)
					_function_data->set_T_node_values(node_id, node_value);
			}  // end of for j
		}  // end of for i
	}  // end of for i_var
...
template <class T_USER_FUNCTION_DATA,
		  ...
		  class T_USER_NON_LINEAR_PROBLEM,
		  class T_USER_NON_LINEAR_SOLUTION>
			  int SingleStepCMP_NonIsoGlobalNCPForm<T_USER_FUNCTION_DATA, T_USER_FEM_PROBLEM, T_USER_NON_LINEAR_PROBLEM, T_USER_NON_LINEAR_SOLUTION>
::solveTimeStep(const NumLib::TimeStep &t_n1)
{
	// print solving information
	INFO("--Solving nonlinear equation for the CMP_NonIsoGlobalNCPForm process. \n");
	
	_nlin_solution->solveTimeStep(t_n1);

    // update the P and X vector
	_function_data->set_P(_nlin_solution->getCurrentSolution(0));
	_function_data->set_X_G_a(_nlin_solution->getCurrentSolution(1));
	_function_data->set_X_G_w(_nlin_solution->getCurrentSolution(2));
	_function_data->set_S(_nlin_solution->getCurrentSolution(3));
	_function_data->set_T(_nlin_solution->getCurrentSolution(4));
