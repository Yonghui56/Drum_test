...
class TemplateTransientDxFEMFunction_2P3CGlobalNCPForm
{
public:
	...
	TemplateTransientDxFEMFunction_2P3CGlobalNCPForm(MeshLib::IMesh* msh,
    ...
    ///
	virtual ~TemplateTransientDxFEMFunction_2P3CGlobalNCPForm()
    {
		_function_data = NULL;
    };
...
void TemplateTransientDxFEMFunction_2P3CGlobalNCPForm<T1, T2, T3, T4>::eval(const SolutionLib::SolutionVector &u_n1, const SolutionLib::SolutionVector &r, SolutionLib::SolutionVector &du)
{
...
void TemplateTransientDxFEMFunction_2P3CGlobalNCPForm<T_DIS_SYS, T_LINEAR_SOLVER, T_FUNCTION_DATA, T_LOCAL_JACOBIAN_ASSEMBLER>::GlobalJacobianAssembler(T_FUNCTION_DATA* _function_data, LinearSolverType & eqsJacobian_global)
{
	const size_t nnodes = _msh->getNumberOfNodes();
	auto& mat_Jacobian = _function_data->get_mat_Jacob();
	//std::cout << mat_Jacobian << std::endl;
	
	for (size_t node_idx_row = 0; node_idx_row < nnodes; node_idx_row++){
		for (size_t node_idx_col = 0; node_idx_col < 7*nnodes; node_idx_col++){
			double const tmp1 =
			    mat_Jacobian.coeff(5 * nnodes + node_idx_row, node_idx_col);
			double const tmp2=
				mat_Jacobian.coeff(6 * nnodes + node_idx_row, node_idx_col);
			eqsJacobian_global.addA(7*node_idx_row +5, node_idx_col, -tmp1);
			eqsJacobian_global.addA(7 * node_idx_row + 6, node_idx_col, -tmp2);
		}
	}
}
