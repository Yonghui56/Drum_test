...
#ifndef SINGLE_STEP_CMP_2P3CGlobalNCPForm_H 
#define SINGLE_STEP_CMP_2P3CGlobalNCPForm_H 
...
>
class SingleStepCMP_2P3CGlobalNCPForm
...
public:
	...
	*/
	SingleStepCMP_2P3CGlobalNCPForm(MyDiscreteSystem*      dis,
	...
	*/
	virtual ~SingleStepCMP_2P3CGlobalNCPForm()
	{
		/*
	...
	* get the 
	...
	of freedom equation ID table
	*/
	DiscreteLib::DofEquationIdTable* getDofEquationIdTable() { return &_dofManager; };

private:
	DISALLOW_COPY_AND_ASSIGN(SingleStepCMP_2P3CGlobalNCPForm);
...
template <
	...
	class T_USER_NON_LINEAR_SOLUTION
>
SingleStepCMP_2P3CGlobalNCPForm<T_USER_FUNCTION_DATA, T_USER_FEM_PROBLEM, T_USER_NON_LINEAR_PROBLEM, T_USER_NON_LINEAR_SOLUTION>
::SingleStepCMP_2P3CGlobalNCPForm(MyDiscreteSystem*                dis,
                           UserFemProblem*                  problem,
						   UserNonLinearSolution*           nlin_solution,
						   UserFunctionData*                function_data)
		...
		for (i = 0; i < var->getNumberOfDirichletBC(); i++) {
			...
			for (j = 0; j < list_bc_nodes.size(); j++)
			{
				...
				if (i_var == 0)
					...
					_function_data->set_X1_node_values(node_id, node_value);
				else if (i_var==2)
					_function_data->set_X2_node_values(node_id, node_value);
				else if (i_var==3)
					_function_data->set_X3_node_values(node_id, node_value);
				else if (i_var == 4)
					_function_data->set_X4_node_values(node_id, node_value);
				else if (i_var == 5)
					_function_data->set_X5_node_values(node_id, node_value);
				else if (i_var==6)
...
template <class T_USER_FUNCTION_DATA,
		  ...
		  class T_USER_NON_LINEAR_PROBLEM,
		  class T_USER_NON_LINEAR_SOLUTION>
			  int SingleStepCMP_2P3CGlobalNCPForm<T_USER_FUNCTION_DATA, T_USER_FEM_PROBLEM, T_USER_NON_LINEAR_PROBLEM, T_USER_NON_LINEAR_SOLUTION>
::solveTimeStep(const NumLib::TimeStep &t_n1)
{
    ...
    _function_data->set_X1(_nlin_solution->getCurrentSolution(1));
	_function_data->set_X2(_nlin_solution->getCurrentSolution(2));
	_function_data->set_X3(_nlin_solution->getCurrentSolution(3));
	_function_data->set_X4(_nlin_solution->getCurrentSolution(4));
	_function_data->set_X5(_nlin_solution->getCurrentSolution(4));
	_function_data->set_S(_nlin_solution->getCurrentSolution(6));
