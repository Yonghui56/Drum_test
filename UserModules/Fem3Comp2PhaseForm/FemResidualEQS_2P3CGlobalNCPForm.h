...
class TemplateTransientResidualFEMFunction_2P3CGlobalNCPForm
...
public:
	...
	TemplateTransientResidualFEMFunction_2P3CGlobalNCPForm(MyDiscreteSystem* sys,
    ...
    ///
	virtual ~TemplateTransientResidualFEMFunction_2P3CGlobalNCPForm()
    {
        _function_data	= NULL;
    };
    ...
    {
		return new TemplateTransientResidualFEMFunction_2P3CGlobalNCPForm
...
template <class T_DIS_SYS, class T_FUNCTION_DATA, class T_LOCAL_RESIDUAL_ASSEMBLER>
void TemplateTransientResidualFEMFunction_2P3CGlobalNCPForm<T_DIS_SYS, T_FUNCTION_DATA, T_LOCAL_RESIDUAL_ASSEMBLER>::eval(const SolutionLib::SolutionVector &u_n1, SolutionLib::SolutionVector &r)
{
    // input, output
...
template <class T_DIS_SYS, class T_FUNCTION_DATA, class T_LOCAL_RESIDUAL_ASSEMBLER>
void TemplateTransientResidualFEMFunction_2P3CGlobalNCPForm<T_DIS_SYS, T_FUNCTION_DATA, T_LOCAL_RESIDUAL_ASSEMBLER>
::GlobalResidualAssembler(T_FUNCTION_DATA * _function_data,
                               SolutionLib::SolutionVector & residual_global)
{
	
	const size_t nnodes = _dis_sys->getMesh()->getNumberOfNodes();
	_vec_Res = MathLib::LocalVector::Zero(7 * nnodes, 1);
	_vec_Res = _function_data->get_vec_Res();
	for (size_t node_idx = 0; node_idx < nnodes; node_idx++){
		residual_global[7 * node_idx+5] = _vec_Res[5 * nnodes+node_idx];
		residual_global[7 * node_idx + 6] = _vec_Res[6 * nnodes + node_idx];
